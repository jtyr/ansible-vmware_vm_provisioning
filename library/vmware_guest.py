...
options:
  ...
  disk:
    ...
    - ' - C(find_vmdk) (bool): Check if the disk exists on the datastore if C(filename) is defined (default C(True)).'
    - '   Set to C(False) if the datastore has a lot of files or if it's too slow to speed up the VM creation. Added in version 2.10.'
...
- name: Create a virtual machine from a template
  vmware_guest:
    ...
    - filename: [datastore1] testvms/testvm_2_1/testvm_2_1.vmdk
      # Disable checking if the VMDK file exists as that might take too long
      find_vmdk: no
    hardware:
      memory_mb: 512
      num_cpus: 6
...
class PyVmomiHelper(PyVmomi):
    ...
    def add_existing_vmdk(self, vm_obj, expected_disk_spec, diskspec, scsi_ctl):
        ...
        if (vm_obj and diskspec.device.backing.fileName != filename) or vm_obj is None:
            if 'find_vmdk' in expected_disk_spec and expected_disk_spec['find_vmdk'] is True:
